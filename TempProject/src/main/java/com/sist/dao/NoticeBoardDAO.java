...
public class NoticeBoardDAO {
	...
	// 공지사항 게시판 전체 리스트
	public List<NoticeBoardVO> boardListData(int type,int page)
	{
		...
		try
		{
			...
			{
				sql="SELECT gnbno,type,subject,name,TO_CHAR(regdate,'YYYY-MM-DD'),hit,num "
			...
			{
				sql="SELECT gnbno,type,subject,regdate,hit "
						+ "FROM god_notice_board_3 "
						+ "WHERE type=? ";
				ps=conn.prepareStatement(sql);
